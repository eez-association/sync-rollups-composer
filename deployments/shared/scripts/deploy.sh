...
# Build all contracts up-front (sync-rollups first, then parent for L2Context).
# Skip openzeppelin formal verification (fv/) which references generated files
# not present in the git checkout. Using --skip avoids mutating the bind-mounted
# contracts directory on the host.
...
ROLLUPS_BYTECODE=$(_bc "$CONTRACTS_DIR/sync-rollups/out/Rollups.sol/Rollups.json")
...
if [ "${MOCK_VERIFIER:-false}" = "true" ]; then
    ...
    VERIFIER_BYTECODE=$(_bc "$CONTRACTS_DIR/sync-rollups/out/tmpECDSAVerifier.sol/tmpECDSAVerifier.json")
