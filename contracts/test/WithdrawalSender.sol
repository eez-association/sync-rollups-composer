...
/// Accepts ETH deposits and can initiate withdrawals via Bridge.bridgeEther(0).
contract WithdrawalSender {
    /// @notice Initiate a withdrawal by calling Bridge.bridgeEther(0)
    function triggerWithdrawal(address bridge, uint256 amount) external {
        IBridge(bridge).bridgeEther{value: amount}(0);
    }
