...
// ── §4f Protocol TX Filtering Tests ────────────────────────────────────

/// Helper: build a test signer and CCM address for filtering tests.
fn test_signer_and_ccm() -> (alloy_signer_local::PrivateKeySigner, Address) {
    let signer: alloy_signer_local::PrivateKeySigner =
        "ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"
            .parse()
            .unwrap();
    let ccm_address = Address::with_last_byte(0xCC);
    (signer, ccm_address)
}

/// Helper: build a setContext tx (not an executeRemoteCall — should always be kept).
fn build_test_set_context_tx(
    signer: &alloy_signer_local::PrivateKeySigner,
    nonce: u64,
) -> TransactionSigned {
    build_set_context_tx(
        1, // l1_block_number
        B256::ZERO,
        Address::with_last_byte(0xAA), // l2_context_address
        signer,
        nonce,
        1, // chain_id
        1, // gas_price
...
/// Helper: build an executeRemoteCall tx targeting the CCM.
fn build_test_execute_remote_call_tx(
    signer: &alloy_signer_local::PrivateKeySigner,
    ccm_address: Address,
    nonce: u64,
    destination: Address,
    ...
    let action = CrossChainAction {
        ...
        rollup_id: U256::from(1),
        destination,
        data: vec![0xd0, 0x9d, 0xe0, 0x8a], // increment() selector
        value: U256::ZERO,
        failed: false,
        source_address: Address::with_last_byte(0x01),
        source_rollup: U256::from(1),
    ...
    build_execute_incoming_tx(&action, ccm_address, signer, nonce, 1, 1).unwrap()
}

/// Helper: build a user tx (not from builder, not targeting CCM).
fn build_test_user_tx(nonce: u64) -> TransactionSigned {
    let signer: alloy_signer_local::PrivateKeySigner =
        "59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d"
            .parse()
            .unwrap();
    build_signed_legacy_tx(
        Some(Address::with_last_byte(0xFF)),
        vec![],
        &signer,
        nonce,
        1,
...
/// Helper: encode a list of txs as RLP bytes.
fn encode_test_txs(txs: &[TransactionSigned]) -> Bytes {
    let mut buf = Vec::new();
...
#[test]
fn test_filter_prefix_counting_partial_consumption() {
    // §4f: keep first N executeRemoteCall txs, discard rest.
    // Block: [setContext, execRemote_E1, execRemote_E2, execRemote_E3, userTx]
    // consumed_trigger_count = 1 → keep E1, discard E2 and E3
    let (signer, ccm) = test_signer_and_ccm();
    let txs = vec![
        build_test_set_context_tx(&signer, 10),
        build_test_execute_remote_call_tx(&signer, ccm, 11, Address::with_last_byte(0x01)),
        build_test_execute_remote_call_tx(&signer, ccm, 12, Address::with_last_byte(0x02)),
        build_test_execute_remote_call_tx(&signer, ccm, 13, Address::with_last_byte(0x03)),
        build_test_user_tx(0),
    ];
    let encoded = encode_test_txs(&txs);

    let filtered = filter_unconsumed_execute_remote_calls(&encoded, ccm, 1, false).unwrap();

    let result: Vec<TransactionSigned> =
        alloy_rlp::Decodable::decode(&mut filtered.as_ref()).unwrap();
    // Should have: setContext + E1 + userTx = 3 txs
    assert_eq!(
        result.len(),
        3,
        "should keep setContext + 1 execRemote + userTx"
    );
    // First tx is setContext (not targeting CCM with execRemote selector)
    assert!(!is_ccm_execute_remote_call(&result[0], ccm));
    // Second tx is the kept executeRemoteCall
    assert!(is_ccm_execute_remote_call(&result[1], ccm));
    // Third tx is the user tx
    assert!(!is_ccm_execute_remote_call(&result[2], ccm));
}

#[test]
fn test_filter_zero_consumption_discards_all_exec_remote() {
    // consumed_trigger_count = 0 → discard ALL executeRemoteCall txs
    let (signer, ccm) = test_signer_and_ccm();
    let txs = vec![
        build_test_set_context_tx(&signer, 10),
        build_test_execute_remote_call_tx(&signer, ccm, 11, Address::with_last_byte(0x01)),
    ...
    let filtered = filter_unconsumed_execute_remote_calls(&encoded, ccm, 0, false).unwrap();

    let result: Vec<TransactionSigned> =
        alloy_rlp::Decodable::decode(&mut filtered.as_ref()).unwrap();
    assert_eq!(result.len(), 2, "should keep only setContext + userTx");
    assert!(!is_ccm_execute_remote_call(&result[0], ccm));
    assert!(!is_ccm_execute_remote_call(&result[1], ccm));
}

#[test]
fn test_filter_full_consumption_keeps_all() {
    // consumed_trigger_count = 2 (all entries) → keep everything
    let (signer, ccm) = test_signer_and_ccm();
    let txs = vec![
        build_test_set_context_tx(&signer, 10),
        build_test_execute_remote_call_tx(&signer, ccm, 11, Address::with_last_byte(0x01)),
    ...
    let filtered = filter_unconsumed_execute_remote_calls(&encoded, ccm, 2, false).unwrap();

    let result: Vec<TransactionSigned> =
        alloy_rlp::Decodable::decode(&mut filtered.as_ref()).unwrap();
    assert_eq!(result.len(), 4, "all txs should be kept");
}

#[test]
fn test_filter_duplicate_action_hash_handled_by_prefix() {
    // Two entries calling the SAME contract with SAME parameters → same actionHash.
    // Prefix counting keeps first K regardless of hash collisions.
    // consumed_trigger_count = 1 → keep first, discard second (even though same hash).
    let (signer, ccm) = test_signer_and_ccm();
    let same_dest = Address::with_last_byte(0x42);
    let txs = vec![
        build_test_set_context_tx(&signer, 10),
        build_test_execute_remote_call_tx(&signer, ccm, 11, same_dest), // E1
        build_test_execute_remote_call_tx(&signer, ccm, 12, same_dest), // E2 (same dest!)
    ];
    let encoded = encode_test_txs(&txs);
    ...
    assert_eq!(
        result.len(),
        2,
        "should keep setContext + first execRemote only"
    );
    assert!(is_ccm_execute_remote_call(&result[1], ccm));

    // Verify nonces: kept tx should have nonce=11 (the first executeRemoteCall)
    use alloy_consensus::Transaction;
    assert_eq!(result[1].nonce(), 11);
}

#[test]
fn test_filter_preserves_load_table() {
    // loadExecutionTable should ALWAYS be kept, even with zero consumption.
    let (signer, ccm) = test_signer_and_ccm();

    // Build a loadTable tx
    let table_entry = CrossChainExecutionEntry {
        state_deltas: vec![],
        action_hash: B256::ZERO,
        next_action: CrossChainAction {
            action_type: CrossChainActionType::Result,
    ...
    let load_table_tx = build_load_table_tx(&[table_entry], ccm, &signer, 11, 1, 1).unwrap();

    let txs = vec![
        build_test_set_context_tx(&signer, 10),
        load_table_tx,
        build_test_execute_remote_call_tx(&signer, ccm, 12, Address::with_last_byte(0x01)),
    ];
    let encoded = encode_test_txs(&txs);

    // Zero consumption: discard executeRemoteCall but keep loadTable
    let filtered = filter_unconsumed_execute_remote_calls(&encoded, ccm, 0, false).unwrap();

    let result: Vec<TransactionSigned> =
        alloy_rlp::Decodable::decode(&mut filtered.as_ref()).unwrap();
    assert_eq!(result.len(), 2, "should keep setContext + loadTable");
    // Verify loadTable is kept (targets CCM but not with execRemote selector)
    use alloy_consensus::Transaction;
    assert_eq!(result[1].to(), Some(ccm));
    assert!(!is_ccm_execute_remote_call(&result[1], ccm));
}

#[test]
fn test_filter_empty_transactions() {
    let result =
        filter_unconsumed_execute_remote_calls(&Bytes::new(), Address::ZERO, 0, false).unwrap();
    assert!(result.is_empty());
}

#[test]
fn test_is_ccm_wrong_selector_not_matched() {
    // A tx targeting CCM with loadTable selector should NOT be identified as executeRemoteCall.
    let (signer, ccm) = test_signer_and_ccm();
    let table_entry = CrossChainExecutionEntry {
        state_deltas: vec![],
        action_hash: B256::ZERO,
    ...
    let load_table_tx = build_load_table_tx(&[table_entry], ccm, &signer, 0, 1, 1).unwrap();
    assert!(
        !is_ccm_execute_remote_call(&load_table_tx, ccm),
        "loadTable tx should NOT match executeRemoteCall check"
    );
}

#[test]
fn test_is_ccm_wrong_target_not_matched() {
    // A tx with executeRemoteCall selector but targeting a DIFFERENT address should not match.
    let (signer, ccm) = test_signer_and_ccm();
    let wrong_ccm = Address::with_last_byte(0xDD); // different from ccm (0xCC)
    let tx = build_test_execute_remote_call_tx(&signer, wrong_ccm, 0, Address::ZERO);
    assert!(
        !is_ccm_execute_remote_call(&tx, ccm),
        "tx targeting wrong address should not match"
    );
    // But it SHOULD match against the actual target
    assert!(is_ccm_execute_remote_call(&tx, wrong_ccm));
}

#[test]
fn test_is_ccm_short_calldata_not_matched() {
    // A tx with < 4 bytes of calldata should not match.
    let (signer, _ccm) = test_signer_and_ccm();
    let tx = build_signed_legacy_tx(
        Some(Address::with_last_byte(0xCC)),
        vec![0x01, 0x02, 0x03], // only 3 bytes
        &signer,
        0,
        1,
        1,
    ...
    .unwrap();
    assert!(!is_ccm_execute_remote_call(
        &tx,
...
#[test]
fn test_is_ccm_empty_calldata_not_matched() {
    let (signer, _ccm) = test_signer_and_ccm();
    let tx = build_signed_legacy_tx(
        Some(Address::with_last_byte(0xCC)),
        vec![],
...
#[test]
fn test_filter_consumed_count_exceeds_actual_keeps_all() {
    // consumed_trigger_count = 100 but only 2 executeRemoteCall txs in block.
    // Should keep all 2 (saturating — no panic).
    let (signer, ccm) = test_signer_and_ccm();
    let txs = vec![
        build_test_set_context_tx(&signer, 10),
        build_test_execute_remote_call_tx(&signer, ccm, 11, Address::with_last_byte(0x01)),
    ...
    // consumed_trigger_count=100, but only 2 executeRemoteCall exist
    let filtered = filter_unconsumed_execute_remote_calls(&encoded, ccm, 100, false).unwrap();

    let result: Vec<TransactionSigned> =
        alloy_rlp::Decodable::decode(&mut filtered.as_ref()).unwrap();
    assert_eq!(result.len(), 4, "all txs kept when count exceeds actual");
}

#[test]
fn test_filter_only_execute_remote_calls_in_block() {
    // Block contains ONLY executeRemoteCall txs (no setContext, no user txs).
    // consumed_trigger_count = 1 → keep first, discard second.
    let (signer, ccm) = test_signer_and_ccm();
    let txs = vec![
        build_test_execute_remote_call_tx(&signer, ccm, 0, Address::with_last_byte(0x01)),
        build_test_execute_remote_call_tx(&signer, ccm, 1, Address::with_last_byte(0x02)),
        build_test_execute_remote_call_tx(&signer, ccm, 2, Address::with_last_byte(0x03)),
    ];
    let encoded = encode_test_txs(&txs);
...
#[test]
fn test_filter_malformed_rlp_returns_error() {
    // Invalid RLP should return Err, not panic.
    let bad_rlp = Bytes::from(vec![0xFF, 0xFE, 0xFD, 0xFC]);
    let result = filter_unconsumed_execute_remote_calls(&bad_rlp, Address::ZERO, 0, false);
    assert!(result.is_err(), "malformed RLP should return Err");
}

#[test]
fn test_filter_with_zero_ccm_address_keeps_all() {
    // When ccm_address is zero, no tx will match (legit txs don't target address zero).
    // All txs should be kept.
    let (signer, ccm) = test_signer_and_ccm();
    let txs = vec![
        build_test_set_context_tx(&signer, 10),
        build_test_execute_remote_call_tx(&signer, ccm, 11, Address::with_last_byte(0x01)),
    ...
    // Filter with Address::ZERO — no tx targets address zero, so nothing is filtered
    let filtered =
        filter_unconsumed_execute_remote_calls(&encoded, Address::ZERO, 0, false).unwrap();

    let result: Vec<TransactionSigned> =
        alloy_rlp::Decodable::decode(&mut filtered.as_ref()).unwrap();
    assert_eq!(result.len(), 3, "no filtering when ccm_address is zero");
}

#[test]
fn test_filter_mixed_base_and_rpc_triggers() {
    // Simulate block with 2 base (L1-fetched) triggers + 2 RPC triggers = 4 total.
    // Base triggers come first and are always kept.
    // consumed_trigger_count = 3 → keep 3 (all 2 base + 1 RPC), discard last RPC.
    let (signer, ccm) = test_signer_and_ccm();
    let txs = vec![
        build_test_set_context_tx(&signer, 10),
        // loadTable (always kept)
        {
            let entry = CrossChainExecutionEntry {
                state_deltas: vec![],
                action_hash: B256::ZERO,
            ...
            build_load_table_tx(&[entry], ccm, &signer, 11, 1, 1).unwrap()
        },
        // Base trigger 1
        build_test_execute_remote_call_tx(&signer, ccm, 12, Address::with_last_byte(0xA1)),
        // Base trigger 2
        build_test_execute_remote_call_tx(&signer, ccm, 13, Address::with_last_byte(0xA2)),
        // RPC trigger 1
        build_test_execute_remote_call_tx(&signer, ccm, 14, Address::with_last_byte(0xB1)),
        // RPC trigger 2
        build_test_execute_remote_call_tx(&signer, ccm, 15, Address::with_last_byte(0xB2)),
        build_test_user_tx(0),
    ];
    let encoded = encode_test_txs(&txs);

    // Keep first 3 executeRemoteCall txs (2 base + 1 RPC), discard last RPC
    let filtered = filter_unconsumed_execute_remote_calls(&encoded, ccm, 3, false).unwrap();

    let result: Vec<TransactionSigned> =
        alloy_rlp::Decodable::decode(&mut filtered.as_ref()).unwrap();
    // setContext + loadTable + 3 executeRemote + userTx = 6
    assert_eq!(result.len(), 6);
    // Count executeRemoteCall txs in result
    let exec_count = result
        .iter()
        .filter(|tx| is_ccm_execute_remote_call(tx, ccm))
        .count();
    assert_eq!(
        exec_count, 3,
        "should keep 3 executeRemoteCall (2 base + 1 RPC)"
    );
}

#[test]
fn test_selector_constants_are_correct() {
    // Verify the selectors match the sol! bindings.
    // executeIncomingCrossChainCall(address,uint256,bytes,address,uint256,uint256[])
    assert_eq!(EXECUTE_REMOTE_CALL_SELECTOR.len(), 4);
    assert_eq!(LOAD_EXECUTION_TABLE_SELECTOR.len(), 4);
    // They should be different
    assert_ne!(
        EXECUTE_REMOTE_CALL_SELECTOR, LOAD_EXECUTION_TABLE_SELECTOR,
        "executeRemoteCall and loadTable must have different selectors"
    );
}
