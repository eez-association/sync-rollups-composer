...
//! `ConsensusEngineHandle`, a `RootProvider` for L1, an `RollupEvmConfig` with a
...
//! [`crate::driver::Driver::build_derived_block`] are NOT reachable via this
//! harness:
//!
...
pub(crate) type HarnessL2Provider = BlockchainProvider<MockNodeTypesWithDB>;
...
pub struct DriverTestHarness {
    ...
    /// it alive. Tests that want to drive engine responses can take() it out.
    #[allow(dead_code)]
    engine_rx: Option<
...
impl DriverTestHarness {
    ...
    pub fn new() -> Self {
        ...
        // channel (rather than erroring synchronously). Tests that explicitly
        // drive the engine protocol can `take_engine_rx()` and script
    ...
    /// Default test config — mirrors `driver_tests::test_config_arc` but kept
    /// local so the two harnesses don't drift. No L1 RPC, no builder, no
    /// proposer, no WS.
    ...
    pub fn assert_recovery_state_cleared(&self) {
        ...
        assert_eq!(
            self.driver.pending_entry_verification_block_for_test(),
            None
        );
        assert_eq!(self.driver.entry_verify_deferrals_for_test(), 0);
    }
