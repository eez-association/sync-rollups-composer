...
| 4 | §4f filtering is per-call prefix counting, never all-or-nothing | Comment in `derivation.rs` and `cross_chain.rs:2237` (`compute_consumed_trigger_prefix`) | `test_compute_consumed_trigger_prefix_*` + proptest `compute_consumed_trigger_prefix_bounded_by_input` / `_monotonic_in_map` | `ConsumedPrefix(usize)` newtype returned by `compute_consumed_trigger_prefix` — callers must `.as_usize()` to escape, monotonicity proptests in `derivation_tests.rs` | Test/gate | 0.3 | ☑ |
...
| 8 | First TRIGGER entry needs `currentState=clean` (post swap-and-pop reorder) | Imperative logic in `attach_chained_state_deltas` (`cross_chain.rs:1520`) | `test_first_trigger_needs_clean_root_after_reorder` | `RevertGroupBuilder::first_trigger_idx()` correctly computed, with dedicated test. **Structurally closed**: group-based `attach_generic_state_deltas` + `PendingL1SubmissionQueue` from Phase 1 ensure `first_trigger_idx` is computed by the group builder, not by callers. | Test/gate | 1.9b | ☑ |
...
| 12 | Multi-call L2→L1 must use scope navigation on Entry 1 | Comment in `table_builder.rs:1612` | `test_l2_to_l1_continuation_uses_scope_return` | `L2ToL1ContinuationBuilder::with_scope_return(scope)` scaffold — `build()` panics if `with_scope_return` was not called. Callers can migrate incrementally. | Compile-time (scaffold: runtime assert) | 1.9c | ☑ |
...
| 16 | **§4f filtering is generic (CrossChainCallExecuted events), NOT Bridge selectors** | `cross_chain.rs:2172` (`identify_trigger_tx_indices`) — already generic, no selectors | `test_filter_uses_event_not_selector` + `ConsumedPrefix` newtype makes consumed count a typed return | `ConsumedPrefix(usize)` newtype returned by `compute_consumed_trigger_prefix` — shares closure with #4. `identify_trigger_tx_indices` uses only `ExecutionConsumed` events, no Bridge ABI dependency. | Test/gate | 0.3 | ☑ |
...
| 18 | **L1 and L2 entry structures must MIRROR** | CLAUDE.md comment | `mirror_loop_tests` (5 canonical cases) + `mirror_property_tests` (proptest: structural symmetry, void result canonicality) in `table_builder_tests.rs` | Mirror tests with `MirrorCase` DSL in `src/test_support/mirror_case.rs` + proptest in `table_builder_tests.rs::mirror_property_tests` | Test/gate | 0.5 | ☑ |
