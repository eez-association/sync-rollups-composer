...
**0.8** ⭐ **Capture baseline** (new, Codex pass 2 insistence). Create `scripts/refactor/capture_baseline.sh` that brings up a clean devnet-eez, runs the canonical E2Es, and saves:
- `postBatch` calldata hex (per postBatch).
- `BatchPosted` and `ExecutionConsumed` logs (with action hashes).
- State roots per L1 and L2 block.
- Tx ordering per block.
- Output in `tests/baseline/<scenario>.json` (committed to the repo).
Scenarios: `bridge`, `crosschain`, `flashloan`, `multi-call-cross-chain`, `test-depth2-generic`. **Without this, 5.7 (replay gate) is impossible.**
*Files:* `scripts/refactor/capture_baseline.sh` (new), `tests/baseline/*.json` (committed)
