...
**Consequence for the builder**: The builder optimistically builds blocks WITH cross-chain entries (state root X). The immediate entry submitted to L1 uses the clean state root (Y). When all entries are consumed on L1, the on-chain root evolves to X — matching the builder's local state. When entries are NOT consumed, the on-chain root stays at Y (or an intermediate X_k for partial consumption). All nodes — builder, fullnodes, and sync — apply protocol tx filtering (§4f) during derivation, discarding `executeRemoteCall` txs (deposits) and L2-to-L1 cross-chain txs for unconsumed entries from the callData before execution. This produces root Y (zero consumption) or X_k (partial consumption) on every node, matching on-chain. The builder detects the mismatch in `flush_to_l1` (or earlier in `verify_local_block_matches_l1`) and triggers a **sibling reorg** (see §8b): it re-derives block N as N' with §4f-filtered txs and swaps it in via `newPayloadV3 + forkchoiceUpdatedV3(head=N'.hash)`. A bare `forkchoiceUpdatedV3(head=ancestor)` cannot rewind a canonicalized block on plain Ethereum engine kind. Once the sibling is canonical, the builder can submit new blocks without a pre_state_root mismatch.
...
- If the builder's locally-built state root differs from the L1-derived state root:
  - **Builder mode — §4f-shaped post-commit divergence** (the derived block carries `DeferredFiltering` metadata and the local EL stateRoot ≠ L1's canonical stateRoot): recovery is a **sibling reorg**, triggered IMMEDIATELY on detection (no deferrals). See §8b. Rationale: L1 has already confirmed which entries were unconsumed; no future L1 block will change the outcome. Each deferral wastes 1–3 blocks of the 64-block reorg-depth budget (§8b).
  - **Builder mode — other mismatches** (L1 reorg, L1 context mismatch, transient race where the user's consumption tx has not yet landed): the builder transitions back to **Sync mode**, rolls back derivation to the mismatched L1 block, clears all pending submissions and cross-chain entries, and re-derives from L1. These mismatches may self-resolve via later L1 inclusion, so the builder defers up to `MAX_ENTRY_VERIFY_DEFERRALS = 3` times in `verify_local_block_matches_l1` before rewinding. Repeated Builder→Sync→Builder cycles are damped with exponential backoff (up to 60s) to prevent tight rewind loops.
...
### 8b. Post-commit divergence recovery via sibling reorg

**Failure mode.** The builder may optimistically canonicalize block `N` on its local execution layer (reth) — including protocol trigger txs for deposits and L2-to-L1 calls — **before** L1 has confirmed which deferred entries were consumed. If some of those entries turn out unconsumed, §4f strips the corresponding protocol txs from the canonical derivation, and the L1-authoritative stateRoot for block `N` differs from reth's locally-canonical stateRoot for the same block number.

**Why bare `forkchoiceUpdatedV3(head=ancestor)` cannot recover.** Per the Engine API specification (Paris / Cancun), `engine_forkchoiceUpdatedV3(head=ancestor)` without `payloadAttributes` is a silent no-op on plain Ethereum engine kind: the EL treats it as an "already-canonical" FCU and returns `VALID` without unwinding any committed block. Reth enforces this explicitly; quoting the op-node equivalent: *"geth cannot wind back a chain without reorging to a new, previously non-canonical, block."* Without a new sibling payload to reorg onto, the builder would livelock: rebuild → rewind (no-op) → detect same mismatch → rebuild.

**Recovery primitive.** The driver uses a **sibling reorg**: re-derive block `N` as `N'` from the same L1 `postBatch` data with §4f filtering applied, then replace the canonical tip via the standard Engine API reorg path:

1. Roll back derivation to the L1 anchor covering block `N`; clear pending submissions.
2. Re-derive `N'` from L1 callData with §4f-filtered txs (via `apply_deferred_filtering`). The result is a sibling to reth's committed `N`: same parent, same block number, different hash, different stateRoot.
3. **State-root assertion (C2 guard)**: before any engine call, assert `built.state_root == expected_root` where `expected_root` is the on-chain canonical root. If the two differ, §4f has a defect and the driver fails loud rather than commit a wrong-root sibling.
4. `engine_newPayloadV3(N')` — must return `VALID`. `SYNCING` is retried with backoff; `INVALID` bails.
5. `engine_forkchoiceUpdatedV3(head=N'.hash)` — reth performs a sibling-block reorg, evicts `N`'s txs back to the pool, and makes `N'` canonical.
6. Broadcast `PreconfirmedMessage::BlockInvalidated { block_number, new_hash }` so subscribed fullnodes evict any cached hash for block `N` and adopt `N'`. External fullnodes without this intra-process channel still converge via the next L1 derivation cycle (L1 is always the tiebreaker).

This is the same pattern used by reth's own `test_testsuite_deep_reorg`, op-node's `consolidateNextSafeAttributes` / `onBuildStart`, and Taiko.

**Trigger sites.** The driver detects §4f-shaped divergence at two points:

- `flush_to_l1` — before draining pending submissions whose root matches `on_chain_root`, scan the window for blocks with `clean_state_root == on_chain_root && state_root != on_chain_root`. The rightmost match (scanned in reverse, per `find_rightmost_sibling_reorg_target`) is queued as the sibling-reorg target, to avoid an earlier block with a coincidental `clean_state_root` match hijacking the recovery.
- `verify_local_block_matches_l1` — when the derived block carries `DeferredFiltering` metadata and its state root does not match the local block's, fast-path to sibling reorg immediately (skip all 3 deferrals). The request is queued on `pending_sibling_reorg` AND `pending_rewind_target` is set so `step_builder` does not fall through to `commit_batch` and overwrite the rollback.

**Depth budget.** Reth caches trie changesets for `CHANGESET_CACHE_RETENTION_BLOCKS = MAX_REORG_DEPTH = 64` blocks. Beyond that window, the state needed to rebuild a sibling at `target_l2_block` has been evicted, and no Engine API call can reconstruct it.

**Safety gate (`MAX_SAFE_REORG_DEPTH = REORG_SAFETY_THRESHOLD = 48`).** At the top of `step_builder`, if `pending_sibling_reorg.is_some()` and `l2_head_number - target_l2_block >= MAX_SAFE_REORG_DEPTH`, the builder **halts block production** and logs a structured error for operator intervention. 48 is 75% of reth's eviction limit — 16 blocks of headroom for a last-ditch sibling reorg or manual recovery. Silent wedging beyond 64 is the worst possible outcome; halting loud is strictly better.

**Why deferring is provably useless for §4f-shaped mismatches.** The `DeferredFiltering` metadata on the derived block is signed by L1 — no future L1 block changes which entries were consumed in the already-confirmed `postBatch`. Waiting 3 deferrals (~14s of exponential backoff) burns 1–3 blocks of the depth budget with zero chance of self-resolution. Fast-path to sibling reorg preserves the budget for genuine transient cases handled elsewhere.

**Relation to L1 reorgs (§7).** L1 reorgs are a different class of mismatch — the L1 block containing `postBatch` itself is no longer canonical. Those are handled by the derivation-cursor rollback in §7, not by sibling reorg. Sibling reorg addresses only the case where L1 is stable and the builder's local EL disagrees with L1's canonical record.
...
- **Sibling-reorg safety gate**: if `pending_sibling_reorg.is_some()` and `l2_head_number - target_l2_block >= MAX_SAFE_REORG_DEPTH` (48), `step_builder` returns early and block production **halts** until the reorg clears or an operator intervenes. This is the "fail loud, don't silently wedge" guarantee (§8b). Halting prevents the builder from ratcheting further past reth's 64-block changeset-cache eviction limit, beyond which the sibling at `target_l2_block` is unrecoverable.
...
- **Builder → Sync**: on state root mismatch (see §8). Derivation rolls back, pending state is cleared. Re-entry to Builder is damped with exponential backoff to prevent tight rewind loops. For §4f-shaped post-commit divergence the rollback is paired with a queued `SiblingReorgRequest` on `pending_sibling_reorg`; `step_sync` then invokes `rebuild_block_as_sibling` (§8b) on the re-derived block. For other mismatches, the rollback is a bare rewind and derivation re-runs normally.
...
20. **Sibling-reorg recovery**: for every confirmed block, the builder's local EL stateRoot MUST equal L1's canonical stateRoot. If a §4f-shaped post-commit divergence is detected and `l2_head_number - target_l2_block < MAX_SAFE_REORG_DEPTH` (48), the driver MUST perform a sibling reorg via `newPayloadV3 + forkchoiceUpdatedV3(head=N'.hash)` (§8b). At or above the threshold, the driver MUST halt block production and log a structured error — silent wedging past reth's 64-block changeset-cache eviction limit is forbidden.
21. **Engine API rewind mechanism**: `engine_forkchoiceUpdatedV3(head=ancestor)` without `payloadAttributes` is NOT a valid rewind on plain Ethereum engine kind — per the Engine API specification it is a silent no-op when `head` already has a committed descendant. The driver MUST use `newPayloadV3` on a freshly-derived sibling followed by `forkchoiceUpdatedV3(head=sibling.hash)` to replace a divergent canonical block (§8b).
22. **State-root assertion before sibling submission**: before committing a sibling block to the EL via `newPayloadV3`, `rebuild_block_as_sibling` MUST assert `built.state_root == expected_root` (the on-chain canonical root for `target_l2_block`). If they differ, the §4f filter has a defect and the builder MUST fail loud rather than commit a wrong-root sibling. Per CLAUDE.md: *"If roots don't match, there is a real bug in derivation or filtering. The builder must keep rewinding until the root cause is fixed."*
...
If `postBatch` succeeds but the trigger tx **reverts** on L1 (e.g., `EtherDeltaMismatch` on one of several triggers, or gas estimation failure), the builder waits for the `postBatch` receipt, then checks all trigger receipts. If any trigger reverted, the divergence is §4f-shaped: reth has canonicalized the entry-bearing block with all protocol txs, but the canonical derivation with §4f filtering strips the unconsumed L2-to-L1 txs. Recovery is a **sibling reorg** (§8b): the driver queues a `SiblingReorgRequest` on `pending_sibling_reorg`, rolls back derivation to the L1 anchor, and `step_sync` swaps the entry-bearing block for its filtered sibling via `newPayloadV3 + forkchoiceUpdatedV3`. The sibling's state root equals the on-chain intermediate root R_k produced by consumed-prefix filtering.
...
**Entry verification deferrals**: After posting a `postBatch` with cross-chain entries, the builder sets an entry verification hold (§4e). When the entry-bearing block's state root does not yet match the L1-derived root (because `ExecutionConsumed` events have not appeared), the builder may defer verification up to `MAX_ENTRY_VERIFY_DEFERRALS = 3` times. Each deferral returns an error that triggers the main loop's exponential backoff (2+4+8 = 14 seconds total), giving L1 time to mine the user's consumption tx.

**§4f-shaped divergence fast-path (no deferrals)**: For mismatches where the derived block carries `DeferredFiltering` metadata (i.e., `derived.filtering = Some(_)`), deferral is provably useless — L1 has already confirmed which entries were consumed and no future block will change that. `verify_local_block_matches_l1` bypasses all 3 deferrals and triggers a **sibling reorg** immediately (§8b). The request is queued on `pending_sibling_reorg` and paired with a `pending_rewind_target = entry_block - 1` so the entry block itself is re-derived with §4f filtering and swapped via `newPayloadV3 + forkchoiceUpdatedV3`.

**Deferral exhaustion → rewind**: For mismatches where `derived.filtering = None` (L1 reorg, L1 context mismatch, transient race where the user's consumption tx has not yet landed), the builder defers up to 3 times, then **rewinds** to Sync mode with target `entry_block - 1`. Derivation re-runs with §4f filtering applied. This produces correct nonces for subsequent blocks. Without rewind, fullnodes would diverge permanently because they derive with filtered txs while the builder's local state has unfiltered nonces.
