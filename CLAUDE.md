...
- **§4f-shaped mismatches trigger sibling-reorg IMMEDIATELY.** No deferrals. Deferring wastes blocks of the 64-block depth budget (reth's `CHANGESET_CACHE_RETENTION_BLOCKS`). The new fast-path in `verify_local_block_matches_l1` detects `derived.filtering = Some(_)` + state root mismatch → sets `pending_sibling_reorg` + `pending_rewind_target` atomically via `apply_sibling_reorg_plan`.
...
563 tests across unit, EVM integration, and E2E. Both the default build and `--features test-utils` compile and pass the full 563 suite. Tests in `*_tests.rs` sibling files. The `e2e_anvil` and `evm_executor` integration tests require `anvil` running locally (and compiled contract artifacts in `contracts/sync-rollups-protocol/out/`). No clippy errors, no `unwrap()` in production code.
